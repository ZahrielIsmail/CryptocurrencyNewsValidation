...
def get_eth_usd_price_on_date(date):
    ...
    eth_data = yf.download('SHIB-USD', start=date, end=date + timedelta(days=1))
...
std_price_movement = 1.5276830927223458e-06  
...
def run():
    theme = ''
    price = 0
    st.header("Prediction Link (Shiba Inu)")
...
# Assigning values to 0
    ...
    if st.button('Submit'):
        ...
        if not article_content.strip():
            ...
            st.write("Current Shib Price (USD)")
            st.write(price)
