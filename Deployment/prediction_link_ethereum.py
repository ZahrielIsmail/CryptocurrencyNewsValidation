...
def get_eth_usd_price_on_date(date):
    ...
    eth_data = yf.download('ETH-USD', start=date, end=date + timedelta(days=1))
...
mean_price_movement = 10.406232155834717  
std_price_movement = 107.04336987848504  

def run():
    theme = ''
    price = 0
    st.header("Prediction Link (Ethereum)")
...
# Assigning values to 0
    ...
    if st.button('Submit'):
        ...
        data = {
            ...
            'Theme_business': 1 if theme == 'business' else 0,
            'Theme_markets': 1 if theme == 'markets' else 0,
            'Theme_policy': 1 if theme == 'policy' else 0,
            'Theme_tech': 1 if theme == 'tech' else 0,
        }
        ...
        if not article_content.strip():
            ...
            st.write("Current ETH Price (USD)")
            st.write(price)
